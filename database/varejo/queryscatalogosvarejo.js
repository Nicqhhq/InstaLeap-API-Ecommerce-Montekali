class Queryscatalogosvarejo {
    ...
    catalogos_criacao_inicial() {
        ...
        with consulta as (
                    ...
                    when prun_unid_codigo in ('100') then concat('ATC-',prun_unid_codigo)
                    ...
                    when prod_dpto_codigo  in ('114','023') then ('MERCEARIA')
                    when prod_dpto_codigo  in ('115','019') then ('BAZAR')
                    ...
                    when prod_dpto_codigo  = '121' then ('SAZONAL') end as categoria,
        ...
        var select = `with consulta as (
            ...
            true as naocontrolaestoque
                ...
                left join grupos on grup_codigo = prod_grup_codigo
                    ...
                    and prod_balanca <> 'N'
                    and prun_prpdv > 0
                   and prod_dpto_codigo not in ('122','123','124','125','126','127','128','129')
                    and prod_datacad >= current_date -5)
                        select sku, cod_store, categoria, preco_regular, estoque, status, concat(categoria,'-',cod_subcat) as subcat, naocontrolaestoque
                            from consulta
                                order by 1
